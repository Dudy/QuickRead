...
public class QuickReadApplication_bak1 extends Application implements
    ...
    @Override
    public void init() {
        initData(DocumentPersistence.getLastDocument());
...
//        getProjectManagementWindow().setSelectedProject(project);
    ...
    public void closeProjectManagement(Document selectedProject) {
        getMainWindow().removeWindow(projectManagementWindow);
        
        this.project = selectedProject;
        List<String> documentList = this.project.getValueListByKey("documentID");
        
        //TODO: just show the first one for now, show all later on !!
        if (documentList != null && !documentList.isEmpty()) {
            initData(DocumentPersistence.loadDocument(Long.parseLong(documentList.get(0))));
        ...
        if (selectedItemIdObject != null) {
            if (selectedItemIdObject instanceof Integer) {
                ...
                if (lastItemIdObject instanceof Integer) {
                    ...
                    Logger.getLogger(QuickReadApplication_bak1.class.getName()).log(Level.SEVERE, "last item id in dataContainer is not of type Integer");
                }
            } else {
                Logger.getLogger(QuickReadApplication_bak1.class.getName()).log(Level.SEVERE, "selected item id is not of type Integer");
            }
        } else {
            Logger.getLogger(QuickReadApplication_bak1.class.getName()).log(Level.SEVERE, "selected item id is null");
        }
